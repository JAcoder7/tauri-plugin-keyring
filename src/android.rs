...
impl<R: Runtime> CredentialBuilderApi for AndroidCredentialBuilder<R> {
    ...
    fn build(&self, target: Option<&str>, service: &str, user: &str) -> Result<Box<Credential>> {
        let credential: AndroidCredential<R> = AndroidCredential::new_with_target(target, service, user, self.plugin_handle.clone()).unwrap();
...
fn default_credential_builder<R:Runtime>(plugin_handle: PluginHandle<R>) -> Box<CredentialBuilder> {
    Box::new(AndroidCredentialBuilder {plugin_handle:plugin_handle})
}
